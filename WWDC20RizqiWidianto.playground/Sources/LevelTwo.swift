...
public class LevelTwo: UIViewController {
    ...
    var source2 = UIImageView()
    var ampereMeter = UIImageView()
    var level2Objective = UILabel()
    var targetValue = 0.5
    ...
    public override func viewDidLoad() {
        ...
        // MARK: To Set Up Lamp
        lamp = UIImageView(frame: CGRect(x: 230, y: 230, width: 50, height: 50))
        lamp.image = UIImage(named: "LampOff.png")
        lamp.contentMode = .scaleAspectFit
        view.addSubview(lamp)
        
        // MARK: To Set Up The First Source
        source = UIImageView(frame: CGRect(x: 140, y: 390, width: 50, height: 50))
        source.image = UIImage(named: "SourceSelected.png")
        source.contentMode = .scaleAspectFit
        view.addSubview(source)
        
        // MARK: To Set Up The Second Source
        source2 = UIImageView(frame: CGRect(x: 140, y: 250, width: 50, height: 50))
        source2.image = UIImage(named: "SourceSelected.png")
        source2.contentMode = .scaleAspectFit
        view.addSubview(source2)
        
        //MARK: Amperemeter
        ampereMeter = UIImageView(frame: CGRect(x: 15, y: 20, width: 200, height: 100))
        ...
        let x_pos_label_level2Objective = 50
        let y_pos_label_level2Objective = 40
        level2Objective = UILabel(frame: CGRect(x: x_pos_label_level2Objective, y: y_pos_label_level2Objective, width: 300, height: 60))
        level2Objective.text = "TARGET CURRENT: \(targetValue) A"
        level2Objective.font = level2Objective.font.withSize(11)
        level2Objective.textColor = #colorLiteral(red: 1, green: 0.3281747699, blue: 0.4034234285, alpha: 1)
        view.addSubview(level2Objective)
   ...
   //MARK: Success Screen
    ...
    @objc func playButtonTapped(_ sender: UIButton){
        tapSound()
        if newComponentItems.count <= 15 && newComponentItems.count >= 10 {
...
//MARK: Extension for Level Two
extension LevelTwo:  UICollectionViewDataSource, UICollectionViewDelegate, UIGestureRecognizerDelegate {
